...
async function loadLegacyTracksForScene({
	...
	const projectDbName = `video-editor-timelines-${projectId}`;
		...
		const adapter = new IndexedDBAdapter<LegacyTimelineData>({ dbName: sceneDbName, storeName: "timeline", version: 1 });

	let data = await adapter.get("timeline");

	if (!data && isMain) {
		const projectAdapter = new IndexedDBAdapter<LegacyTimelineData>({ dbName: projectDbName, storeName: "timeline", version: 1 });
...
async function loadMediaTypesById({
	...
	if (mediaIds.length === 0) {
		...
		const mediaMetadataAdapter = new IndexedDBAdapter<MediaAssetData>({ dbName: `video-editor-media-${projectId}`, storeName: "media-metadata", version: 1 });
